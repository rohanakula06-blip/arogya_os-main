...
// In-memory + persistent blob cache so OCR and pipeline never fail even if RLS blocks remote storage
const blobCache = new Map<string, Blob>();
const LOCAL_REPORTS_KEY = "arogya_local_reports_v1";
const LOCAL_METRICS_KEY = "arogya_local_health_metrics_v1";

export function getLocalReports(): ReportRow[] {
  try {
...
export function saveLocalReportRow(report: ReportRow): void {
  try {
...
/**
 * Uploads the file to storage (with fallback) and creates the medical_reports row.
 */
export async function uploadMedicalReport(
  params: UploadMedicalReportParams,
): Promise<UploadedReport> {
  const reportId = crypto.randomUUID();
  const objectPath = `${params.userId}/${reportId}-${params.file.name}`;
  const reportDate = params.reportDate || new Date().toISOString().slice(0, 10);
  const nowIso = new Date().toISOString();

  // 1. Always cache file blob in memory so OCR pipeline can access it immediately
  blobCache.set(objectPath, params.file);
  blobCache.set(reportId, params.file);

  // 2. Persist in local reports store
  const localRow: ReportRow = {
    id: reportId,
  ...
  saveLocalReportRow(localRow);

  // 3. Attempt Supabase Storage upload and table insert (best-effort with graceful fallback)
  try {
    ...
    try {
      ...
      console.warn("[reports] Supabase storage upload note (using cached local blob):", storageErr);
    }
    ...
    console.warn("[reports] Supabase remote sync note (proceeding with local pipeline):", err);
  }
...
export async function updateReportProcessingStatus(
  ...
  const found = localList.find((r) => r.id === reportId);
  if (found) {
    found.processing_status = status;
    saveLocalReportRow(found);
  }
  ...
  try {
    const { error } = await getSupabase()
      ...
      .eq("id", reportId);
    if (error) {
      console.warn(`[reports] remote status update note (${status}):`, error.message);
    }
  } catch {
    // ignore
  }
...
/** Downloads the uploaded object back from private storage or local blob cache. */
export async function downloadReportFile(fileUrl: string): Promise<Blob> {
  // 1. Check local blob cache first for instant retrieval
  if (blobCache.has(fileUrl)) {
    return blobCache.get(fileUrl)!;
  }
  ...
  // If still in cache under alternate key
  for (const [key, blob] of blobCache.entries()) {
    if (fileUrl.includes(key) || key.includes(fileUrl)) {
      return blob;
    }
  }

  throw new Error("Report document file could not be retrieved.");
}

/** Persists the extracted OCR text. */
...
/**
 * Persists parsed metrics into health_metrics with local fallback.
 */
export async function saveHealthMetrics(
  ...
  try {
    const existingRaw = localStorage.getItem(LOCAL_METRICS_KEY);
    const existingMetrics: any[] = existingRaw ? JSON.parse(existingRaw) : [];
    const reportMetrics = metrics.map((m) => ({
      report_id: reportId,
    ...
    existingMetrics.push(...reportMetrics);
    localStorage.setItem(LOCAL_METRICS_KEY, JSON.stringify(existingMetrics));
  } catch {
    // ignore
  }

  // Try Supabase health_metrics table
  try {
    const supabase = getSupabase();
    ...
    if (!insertError) {
      return { inserted: metrics.length, duplicates: 0 };
    }
  } catch {
    // ignore
  }
...
export async function fetchMyReports(): Promise<ReportRow[]> {
  ...
  try {
    ...
    if (!error && data) {
      ...
      const combined = [...data, ...localList.filter((l) => !remoteIds.has(l.id))];
      combined.sort((a, b) => new Date(b.created_at).getTime() - new Date(a.created_at).getTime());
      return combined as ReportRow[];
    }
  } catch {
