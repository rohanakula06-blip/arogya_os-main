...
function StatCard({
  ...
  return (
    <motion.div
      ...
      className={`glass-card relative overflow-hidden rounded-3xl p-5 border border-border/80 shadow-xs transition-all duration-300 hover:-translate-y-1 hover:shadow-lg ${borderHover}`}
    >
      ...
      <div className="flex items-center justify-between gap-2">
        ...
        <span className={`flex size-9 shrink-0 items-center justify-center rounded-xl bg-gradient-to-br ${gradient} text-white shadow-md shadow-black/10`}>
          <Icon className="size-4.5" />
        </span>
      </div>
      <p className="tnum mt-3 truncate font-mono text-2xl font-bold tracking-tight text-foreground">
        {value}
      </p>
  ...
  return (
    ...
    >
      ...
      <div className="mt-4 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <StatCard icon={FileText} label="Total Reports" value={String(newestFirst.length)} gradient="from-sky-500 to-blue-600" borderHover="hover:border-sky-500/50" delay={0} />
        <StatCard icon={FlaskConical} label="Metrics Available" value={String(allMetrics?.length ?? 0)} sub="stored readings" gradient="from-teal-500 to-emerald-600" borderHover="hover:border-teal-500/50" delay={0.04} />
        <StatCard icon={CalendarClock} label="Latest Upload" value={latestReport ? formatTimestamp(latestReport.createdAt) : "—"} gradient="from-violet-500 to-indigo-600" borderHover="hover:border-violet-500/50" delay={0.08} />
        <StatCard icon={TrendingUp} label="Avg Hemoglobin" value={`${formatNumber(stats.avgHemoglobin)} g/dL`} gradient="from-rose-500 to-red-600" borderHover="hover:border-rose-500/50" delay={0.12} />
        <StatCard icon={TrendingUp} label="Avg Blood Sugar" value={`${formatNumber(stats.avgBloodSugar)} mg/dL`} gradient="from-amber-500 to-orange-600" borderHover="hover:border-amber-500/50" delay={0.16} />
        <StatCard icon={TrendingUp} label="Avg Cholesterol" value={`${formatNumber(stats.avgCholesterol)} mg/dL`} gradient="from-purple-500 to-indigo-600" borderHover="hover:border-purple-500/50" delay={0.2} />
        <StatCard icon={TrendingUp} label="Highest Blood Sugar" value={`${formatNumber(stats.highestBloodSugar)} mg/dL`} gradient="from-orange-500 to-rose-600" borderHover="hover:border-orange-500/50" delay={0.24} />
        <StatCard icon={TrendingUp} label="Lowest Hemoglobin" value={`${formatNumber(stats.lowestHemoglobin)} g/dL`} gradient="from-emerald-500 to-teal-600" borderHover="hover:border-emerald-500/50" delay={0.28} />
      </div>
