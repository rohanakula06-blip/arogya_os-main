...
const RISK_TONE_DOTS: Record<RiskTone, string> = {
  green: "bg-[#16A34A]",
  blue: "bg-[#0284C7]",
  amber: "bg-[#E08600]",
  orange: "bg-[#E08600]",
...
const RISK_TO_CLINICAL_STATUS: Record<RiskTone, string> = {
  green: "normal",
  blue: "info",
  amber: "watch",
  orange: "watch",
...
function RiskBadge({ tone }: { tone: RiskTone }) {
  return (
    <ClinicalStatusPill
      status={RISK_TO_CLINICAL_STATUS[tone] || "info"}
      label={RISK_TONE_LABELS[tone]}
    />
  );
}
          ...
          <div className="flex flex-wrap items-center gap-2" aria-label="Risk indicator legend">
            {(["green", "blue", "amber", "orange", "red"] as RiskTone[]).map((tone) => (
              <span key={tone} className="flex items-center gap-1.5 text-[10px] text-muted-foreground">
                <span className={`size-2 rounded-full ${RISK_TONE_DOTS[tone]}`} />
